...
$dbIllustData = getIllustList($headNum, $sort, $showNum, '', $_SESSION['user_id']);
//ユーザーデータを取得
...
<body>
      ...
      <section class="range-menu">
        <form action="" method="post" class="illust-or-bookmark">
          <ul class="order_select">
            <li>
              <a href="mypage.php?sort=date<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>" class="current">イラスト</a>
            </li>
            <li>
              <a href="mypageBookmark.php?sort=date<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>">ブックマーク</a>
            </li>
          </ul>
        </form>
      </section>
  ...
  <ul class="order_select">
    <li>
      <a href="mypage.php?sort=date<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>" class="<?php echo getCurrentSort($sort); ?>">投稿順</a>
    </li>
    <li>
      <a href="mypage.php?sort=bookmark<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>" class="<?php if($sort === 2) echo 'current' ?>">人気順</a>
    </li>
<!--
    <li>
