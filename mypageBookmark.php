...
debug('================================');
debug('マイページのブックマーク mypageBookmark.php');
debug('================================');
debugLogStart();
...
$dbIllustData = getIllustBookmarkList($headNum, $sort, $showNum, $_SESSION['user_id']);
//ユーザーデータを取得
...
<body>
      ...
      <section class="range-menu">
        <form action="" method="post" class="illust-or-bookmark">
          <ul class="order_select">
            <li>
              <a href="mypage.php?sort=date<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>">イラスト</a>
            </li>
            <li>
              <a href="mypageBookmark.php?sort=date<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>" class="current">ブックマーク</a>
            </li>
          </ul>
        </form>
      </section>
  ...
  <ul class="order_select">
    <li>
      <a href="mypageBookmark.php?sort=date<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>" class="<?php echo getCurrentSort($sort); ?>">投稿順</a>
    </li>
    <li>
      <a href="mypageBookmark.php?sort=bookmark<?php if(!empty($searchWord)) echo ('&search-word=' .$searchWord); ?>" class="<?php if($sort === 2) echo 'current' ?>">人気順</a>
    </li>
    <!--
<li>
  ...
  <div class="my-pic">
    <div class="pic-container">
      ...
      <div class="pic-flex">
        ...
        ?>
        <div class="picture">
          <a href="illustDetail.php<?php echo (!empty(appendGetParam())) ? appendGetParam() . '&i_id=' .$value['id'] : '?i_id='.$value['id']; ?>" class="author"><img src="<?php echo sanitize($value['pic']); ?>"></a>
          <div class="pic-info">
            ...
            <div><a href="illustDetail.php<?php echo (!empty(appendGetParam())) ? appendGetParam() . '&i_id=' .$value['id'] : '?i_id='.$value['id']; ?>" class="author author-illust"><span><?php echo sanitize($value['username']); ?></span></a></div>
          </div>
        </div>
        <?php 
        endforeach;
